...
public class GenerateInvoicesReportTest : BarberBossIClassFixture
{
    ...
    public GenerateInvoicesReportTest(CustomWebApplicationFactory webApplicationFactory) : base(webApplicationFactory)
    {
    ...
    {
        var result = await DoGet(requestUri: $"{METHOD}/pdf?month={_invoiceDate:Y}", token: _adminToken);
    ...
    {
        var result = await DoGet(requestUri: $"{METHOD}/excel?month={_invoiceDate:Y}", token: _adminToken);
